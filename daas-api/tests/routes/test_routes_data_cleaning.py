...
def test_routes_data_cleaning_index(test_client: FlaskClient):
    # Test invalid JSON
    res = test_client.post("/data-cleaning", follow_redirects=True,
                           data="Hello")
    ...
    assert res.data == b"Data Cleaning job submitted"
...
@mongomock.patch(servers=(("0.0.0.0", 27017), ))
def test_routes_data_cleaning_progress(test_client: FlaskClient):
    # Test missing ID
    res = test_client.get("/data-cleaning/progress", follow_redirects=True)
    assert res.status_code == 400
...
@mongomock.patch(servers=(("0.0.0.0", 27017), ))
def test_routes_data_cleaning_job(test_client: FlaskClient):
    # Test missing ID
    job = "1"
    res = test_client.get(f"/data-cleaning/{job}", follow_redirects=True)
    assert res.status_code == 404
    assert res.data == b"Job ID doesn't exist"
